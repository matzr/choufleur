          ...
          function($scope, $http, socket, $timeout, session) {
            ...
            function init() {
              ...
              $http.get('/samples/' + session.token + '/' + $scope.sensorId + '/' + $scope.dtFrom + '/' + $scope.dtTo).success(function(samples) {
                ...
                if (samples.length > 0) {
                $('.pictures').scrollLeft(($('.pictures-container').width() - $('.pictures').width()));
              ...
              if ((sample.sensor_id === $scope.sensorId) && (start > $scope.dtFrom) && (start < $scope.dtTo)) {
                if ($scope.samples) {
                  ...
                  if (audioSample) {
                    ...
                    var newOffset = ($('.pictures-container').width() - $('.pictures').width()) + 50;
                    $timeout(function () { $('.pictures').animate({scrollLeft: newOffset}, 200); }, 500);
            ...
            $scope.openPhoto = function(sample) {
                $scope.modalShown = true;
                $scope.selectedPictureSample = sample;
            }
